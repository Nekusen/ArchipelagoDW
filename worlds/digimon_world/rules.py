...
def _set_completion_condition(world: DigimonWorldWorld) -> None:
    ...
    History note: a previous version of this rule also required all 3
    ``Progressive Item Shop`` copies as a "difficulty floor" so AP
    fill would route the shop items to reachable spots before Final
    Battle. The user reverted that requirement — the shop isn't
    strictly necessary to beat Machinedramon, so AP shouldn't pretend
    it is.
    """

...
| `notification_top_field.state` | Field, idle -- unattended, on `work\dw1_re\notification_top.bin` (shipped patch + top-banner renderer) | The top-of-screen banner (`renderMapNameAp`, the shipped position) resident; mailbox zero. |
| `gekomon_talk.state` (CAPTURED 2026-08-29; also `gekomon_n3.state` / `gekomon_n3_fired.state` on `gekomon.bin`) | Vanilla ISO, screen 141 `GKYO02`, tamer beside `.MAP` record 3 (532, 0, -1195), trigger 205 clear, no dialog -- `dw1_warp_state.py --map 141` + the entity teleport recipe; one input: CROSS | The Gekomon location splice (Script 135 vm 5562): bit 780 set only through the joined dialog. |
| `ogre_tunn02_after_sbc.state` (CAPTURED 2026-08-29) | Vanilla ISO, screen 25 TUNN02 exit 0, `--set-trigger 150` (234/235/175 clear); one input: walk south onto z in [700,800) | Ogremon B4 cutscene with the bandits (control). With the live MAPHEAD poke `0x1B26E2 = EB` first it reproduces the retired write's hang -- record the blocking op. |
| `ogre_ogre03_nanimon.state` (CAPTURED 2026-08-29) | Screen 48 OGRE03 exit 0, `--set-trigger 150` (175/334 clear); walk north into z >= 1300 | Fortress cutscene with Nanimon co-present (decides guard G1; then poke `0x1B2B94 = AF`). |
| `ogre_ogre11_ferry_first.state` (CAPTURED 2026-08-29) | Screen 143 OGRE11 exit 0, 150/175/224 clear; fires on load | B3 before B2 (the AP ferry order) works with vanilla bytes; 150 set, Whamon joins. |
| `ogre_tunn04_after_ogremon.state` (CAPTURED 2026-08-29) | Screen 27 TUNN04, `--set-trigger 234` (140 clear, pstat(17) = 0); talk to Drimogemon | Guard G2: without it "I fixed the entrance sign", no fight, no 140; with the poke `0x1B9EFA = 8C 00` (after `find 28`) the berserk fight runs. |

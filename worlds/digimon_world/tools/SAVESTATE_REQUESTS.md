...
| `ogre_tunn02_after_sbc.state` | Vanilla ISO, screen 25 TUNN02 exit 0, `--set-trigger 150` (234/235/175 clear); one input: walk south onto z in [700,800) | Ogremon B4 cutscene with the bandits (control). With the live MAPHEAD poke `0x1B26E2 = EB` first it reproduces the retired write's hang -- record the blocking op. |
| `ogre_ogre03_nanimon.state` | Screen 48 OGRE03 exit 0, `--set-trigger 150` (175/334 clear); walk north into z >= 1300 | Fortress cutscene with Nanimon co-present (decides guard G1; then poke `0x1B2B94 = AF`). |
| `ogre_ogre11_ferry_first.state` | Screen 143 OGRE11 exit 0, 150/175/224 clear; fires on load | B3 before B2 (the AP ferry order) works with vanilla bytes; 150 set, Whamon joins. |
| `ogre_tunn04_after_ogremon.state` | Screen 27 TUNN04, `--set-trigger 234` (140 clear, pstat(17) = 0); talk to Drimogemon | Guard G2: without it "I fixed the entrance sign", no fight, no 140; with the poke `0x1B9EFA = 8C 00` (after `find 28`) the berserk fight runs. |
